...
if (strpos($wp->request, 'verify-email') !== false) {
    ...
    if (isset($_GET['verification_key']) && isset($_GET['email']) 
        ...
        if (isset($user) && $user->emailVerificationKey == $verification_key) {
            ...
            error_log($updated === 0);
            error_log($updated >= 0);
            wp_safe_redirect('https://sailhousingsolutions.org/success-message?title=Thank you, your email has been verified.&message=<a href="https://sailhousingsolutions.org/user">Click here to go to your profile page.</a>');
            exit;
        } else {
